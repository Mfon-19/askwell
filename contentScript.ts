...
type EditableField = HTMLTextAreaElement | HTMLElement;
...
function findInputs(): EditableField[] {
  // visible textareas on other sites
  const visibleTAs = Array.from(
    document.querySelectorAll<HTMLTextAreaElement>("textarea")
  ).filter((t) => t.offsetParent !== null);

  // ChatGPT’s live editor
  const proseMirrors = Array.from(
...
function injectBadge(input: EditableField): void {
  if (input.parentElement?.querySelector(".askwell-badge")) return;
  ...
  badge.addEventListener("click", (e: Event) => {
    ...
    if (form) {
      ...
      const hiddenTA = form.querySelector<HTMLTextAreaElement>("textarea");
      if (hiddenTA) {
      ...
      const prose = form.querySelector<HTMLElement>(
        'div.ProseMirror[contenteditable="true"]'
      );
      if (prose) {
    ...
    for (const m of mutations) {
      m.addedNodes.forEach((node) => {
        ...
        if (node.matches('textarea,[contenteditable="true"]')) {
          injectBadge(node as EditableField);
        }

        // Nested matches
        Array.from(
          node.querySelectorAll<EditableField>(
